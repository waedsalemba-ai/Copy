...
class Database {
  ...
  private saveData(): void {
    ...
    if (this.saveTimeout || this.saveInFlight) return;
    this.saveTimeout = setTimeout(() => {
      this.saveTimeout = null;
      this.flushDataAsync().catch((err) => console.error('Failed to write store.json:', err));
    }, 200);
  }

  private async flushDataAsync(): Promise<void> {
    ...
    try {
      ...
      await fs.promises.writeFile(tempFile, JSON.stringify(this.data), 'utf-8');
      await fs.promises.rename(tempFile, DB_FILE);
    } finally {
      this.saveInFlight = false;
      if (this.saveQueued && !this.saveTimeout) {
        this.saveTimeout = setTimeout(() => {
          this.saveTimeout = null;
          this.flushDataAsync().catch((err) => console.error('Failed to write store.json:', err));
        }, 200);
  ...
  getActivePaperPosition(walletAddress: string, tokenMint: string): PaperPosition | undefined {
    return this.data.paperPositions.find(
      (p) =>
        p.sourceWalletAddress.toLowerCase() === walletAddress.toLowerCase() &&
        p.tokenMint === tokenMint &&
