...
export class LaserStreamService extends EventEmitter {
  ...
  private async pollWallets(): Promise<void> {
    ...
    try {
      const connection = rpcService.getConnection();
      // Poll wallets concurrently, but preserve ordering within each wallet.
      // The cursor is wallet-scoped because one transaction may involve several
      // monitored wallets and must be classified once per wallet.
      await Promise.all(Array.from(this.monitoredWallets).map(async (address) => {
        try {
          ...
          const cursor = this.lastSeenSignatureByWallet.get(address.toLowerCase());
          ...
          for (const sigInfo of unseen) {
            await this.handleLogNotification(address, sigInfo.signature);
          }

          if (sigInfos.length > 0) {
            this.lastSeenSignatureByWallet.set(address.toLowerCase(), sigInfos[0].signature);
            if (cursor && cursorIndex < 0) {
              console.warn(`[LaserStream] Poll cursor gap for ${address}; processed latest ${sigInfos.length} signatures`);
            }
          }
        } catch {
          // Ignore transient wallet polling errors; retain the cursor so the
          // failed wallet is retried on the next cycle.
        }
      }));
    } catch {
