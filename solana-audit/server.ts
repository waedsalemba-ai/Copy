...
async function startServer() {
  ...
  app.post('/api/wallets', async (req, res) => {
    ...
    try {
      solBalance = await Promise.race([
        rpcService.getSolBalance(address),
        new Promise<number>((resolve) => setTimeout(() => resolve(0), 5000)),
      ]);
    } catch {
      res.status(502).json({ error: 'Unable to read wallet balance from RPC' });
      return;
    }
  ...
  app.post('/api/settings', (req, res) => {
    ...
    for (const key of ['rpcUrl', 'laserstreamApiKey', 'laserstreamEndpoint', 'minTradeAlertValueUsd', 'webhookUrl']) {
      if (key in incoming) updates[key] = incoming[key];
    }
    for (const key of ['rpcUrl', 'laserstreamEndpoint', 'webhookUrl']) {
      if (typeof updates[key] === 'string' && updates[key]) {
        try {
          const urls = key === 'rpcUrl' ? (updates[key] as string).split(',').map((value) => value.trim()).filter(Boolean) : [updates[key] as string];
          for (const value of urls) {
            const parsed = new URL(value);
            if (!['http:', 'https:', 'ws:', 'wss:'].includes(parsed.protocol)) throw new Error('invalid protocol');
          }
        } catch {
          res.status(400).json({ error: `${key} must contain valid URL(s)` });
          return;
        }
      }
    }
    if ('minTradeAlertValueUsd' in updates && (typeof updates.minTradeAlertValueUsd !== 'number' || (updates.minTradeAlertValueUsd as number) < 0)) {
  ...
  // Development-only acceptance and destructive operations.
