...
export class RpcService {
  ...
  private getConfiguredEndpoints(): string[] {
    ...
    const merged = [...fromSettings, ...config.rpcFallbackUrls];
  ...
  /** Strict quote path for paper execution. Unlike getTokenMetadata, this
   * never falls back to DexScreener or a fabricated value. */
  ...
  private async resolveTokenInfo(tokenMint: string, staticMeta: StaticTokenMeta | undefined): Promise<TokenInfo> {
    ...
    if (!info) {
      ...
      info = {
        ...
        // Never invent an executable price. Paper execution must be blocked
        // when all quote providers are unavailable.
        priceSol: lastKnownPrice ?? 0,
      };
    }
