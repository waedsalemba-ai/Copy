...
export const PaperTradingPanel: React.FC = () => {
  ...
  const fetchPaperData = async () => {
    try {
      const [accRes, posRes, trdRes, setRes, buyEntrySetRes, watchlistRes] = await Promise.all([
        ...
        fetch('/api/paper/settings').then((r) => r.json()),
        fetch('/api/buy-entry/settings').then((r) => r.json()),
        fetch('/api/buy-entry/watchlist').then((r) => r.json()),
      ]);
  ...
  return (
    <div className="space-y-3 font-sans relative">
      <div className="rounded border border-[#f59e0b]/50 bg-[#f59e0b]/10 px-3 py-2 text-[11px] font-mono uppercase text-[#fbbf24]">
        PAPER TRADING ONLY · MODE: PAPER · NO WALLET SIGNING OR ON-CHAIN EXECUTION
      </div>
      {/* Toast Banner */}
      {resetSuccessMsg && (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
        <div className="p-3 rounded bg-[#18181b] border border-[#27272a] shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between text-[#71717a] text-[10px] uppercase font-mono">
            <span>Paper Available Balance</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
          <div className="flex flex-col">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-[11px] text-[#fafafa] font-mono">
          ...
          <div className="flex flex-col">
            ...
            <input
              type="number"
              value={settings?.fixedSolAmountPerTrade || 0.1}
              onChange={(e) => handleUpdateSettings({ fixedSolAmountPerTrade: parseFloat(e.target.value) })}
              step={0.01}
              min={0.01}
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              onChange={(e) => handleUpdateSettings({ simulatedSlippageBps: parseFloat(e.target.value) })}
              step={1}
              min={0}
              className="px-2 py-1 rounded bg-[#09090b] border border-[#27272a] text-[#fafafa] font-mono text-xs focus:outline-none focus:border-[#00FF88]"
            />
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              onChange={(e) => handleUpdateSettings({ startingVirtualSolBalance: parseFloat(e.target.value) })}
              step={0.1}
              min={0.1}
              className="px-2 py-1 rounded bg-[#09090b] border border-[#27272a] text-[#fafafa] font-mono text-xs focus:outline-none focus:border-[#00FF88]"
            />
